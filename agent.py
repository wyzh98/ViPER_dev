...
class Agent:
    def __init__(self, id, policy_net, node_manager, device='cpu', plot=False):
        ...
        # topological graph
        self.cliques, self.topological_node_coords, self.topological_adjacent_matrix_padded = None, None, None

        # ground truth graph (only for critic)
        self.true_node_coords, self.true_adjacent_matrix, self.true_topological_adjacent_matrix_padded = None, None, None
        self.true_cliques, self.true_topological_node_coords = None, None
    ...
    def get_observation(self, pad=True):
        node_coords = self.local_node_coords.reshape(-1, 2)
        topological_node_coords = self.topological_node_coords.reshape(-1, 2)
        node_safe_utility = self.safe_utility.reshape(-1, 1)
        node_uncovered_safe_utility = self.uncovered_safe_utility.reshape(-1, 1)
        node_guidepost = self.guidepost.reshape(-1, 1)
        node_signal = self.signal.reshape(-1, 1)
        node_occupancy = self.occupancy.reshape(-1, 1)
        edge_mask = self.local_adjacent_matrix
        topological_edge_mask = self.topological_adjacent_matrix_padded
        current_edge = self.local_neighbor_indices
        current_traversable_edge = self.traversable_indices
        n_node = node_coords.shape[0]

        node_clique_center = np.zeros_like(node_guidepost)
        all_node_coords_to_check = node_coords[:, 0] + node_coords[:, 1] * 1j
        topological_node_coords_to_check = topological_node_coords[:, 0] + topological_node_coords[:, 1] * 1j
        for i, coords in enumerate(all_node_coords_to_check):
            if coords in topological_node_coords_to_check:
                node_clique_center[i] = 1
        node_clique_center = node_clique_center.reshape(-1, 1)

        current_node_coords = self.local_node_coords[self.current_local_index]
        ...
        node_uncovered_safe_utility = node_uncovered_safe_utility / 30

        node_inputs = np.concatenate((node_coords, node_safe_utility, node_uncovered_safe_utility,
                                      node_guidepost, node_signal, node_occupancy, node_clique_center), axis=1)
        node_inputs = torch.FloatTensor(node_inputs).unsqueeze(0).to(self.device)

        if pad:
            assert node_coords.shape[0] < LOCAL_NODE_PADDING_SIZE, print(node_coords.shape[0])
            input_padding = torch.nn.ZeroPad2d((0, 0, 0, LOCAL_NODE_PADDING_SIZE - n_node))
            node_inputs = input_padding(node_inputs)
        ...
        current_index = torch.tensor([self.current_local_index]).reshape(1, 1, 1).to(self.device)
        ...
        current_traversable_edge = torch.tensor(current_traversable_edge).unsqueeze(0).to(self.device)
        edge_padding_mask = torch.ones_like(current_edge).to(self.device)
        edge_padding_mask[torch.isin(current_edge, current_traversable_edge)] = 0
        if pad:
            padding0 = torch.nn.ConstantPad1d((0, LOCAL_K_PADDING_SIZE - k_size), 0)
            current_edge = padding0(current_edge)
            padding1 = torch.nn.ConstantPad1d((0, LOCAL_K_PADDING_SIZE - k_size), 1)
            edge_padding_mask = padding1(edge_padding_mask)
        current_edge = current_edge.unsqueeze(-1)
        edge_padding_mask = edge_padding_mask.unsqueeze(0)

        return [node_inputs, node_padding_mask, edge_mask, topological_edge_mask, current_index, current_edge, edge_padding_mask]

    def get_state(self):
        true_node_coords = self.true_node_coords.reshape(-1, 2)
        true_topological_node_coords = self.true_topological_node_coords.reshape(-1, 2)
        ...
        true_edge_mask = self.true_adjacent_matrix
        ...
        true_node_clique_center = np.zeros_like(true_node_guidepost)
        all_node_coords_to_check = true_node_coords[:, 0] + true_node_coords[:, 1] * 1j
        topological_node_coords_to_check = true_topological_node_coords[:, 0] + true_topological_node_coords[:, 1] * 1j
        for i, coords in enumerate(all_node_coords_to_check):
            if coords in topological_node_coords_to_check:
                true_node_clique_center[i] = 1
        true_node_clique_center = true_node_clique_center.reshape(-1, 1)
        ...
        state_node_inputs = np.concatenate((true_node_coords, true_node_safe_utility, true_node_uncovered_safe_utility,
                                            true_node_guidepost, true_node_signal, true_node_occupancy, true_node_clique_center), axis=1)
        ...
        node_padding = torch.ones((1, 1, LOCAL_NODE_PADDING_SIZE - n_true_node), dtype=torch.int16).to(self.device)
        state_node_padding_mask = torch.cat((state_node_padding_mask, node_padding), dim=-1)

        state_edge_mask = torch.tensor(true_edge_mask).unsqueeze(0).to(self.device)
        state_topological_edge_mask = torch.tensor(true_topological_edge_mask).unsqueeze(0).to(self.device)
        padding = torch.nn.ConstantPad2d((0, LOCAL_NODE_PADDING_SIZE - n_true_node, 0, LOCAL_NODE_PADDING_SIZE - n_true_node), 1)
        state_edge_mask = padding(state_edge_mask)
        state_topological_edge_mask = padding(state_topological_edge_mask)

        return [state_node_inputs, state_node_padding_mask, state_edge_mask, state_topological_edge_mask]
    ...
    def save_observation(self, local_observation):
        node_inputs, node_padding_mask, edge_mask, top_edge_mask, current_index, current_edge, edge_padding_mask = local_observation
        ...
        self.add(self.episode_buffer, 'edge_mask', edge_mask.bool())
    ...
    def save_next_observations(self, local_observation, next_node_index_list):
        ...
        self.episode_buffer['next_edge_mask'] += edge_mask.bool()
    ...
    def save_state(self, state):
        state_node_inputs, state_node_padding_mask, state_edge_mask, state_top_edge_mask = state
