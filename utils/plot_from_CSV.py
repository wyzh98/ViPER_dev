...
def plot_training_curve2():
    filetype = '.csv'
    file_list = []
    legend_list = []
    method_list = []

    for _, _, files in os.walk(f'results/ablation-encoder'):
        for f in files:
            if filetype in f:
                file_list.append(f)
    file_list.sort(reverse=False)
    ...
    for cnt, tag in enumerate(['Max']):
        ...
        for i, csv_file in enumerate(file_list):
            if tag in csv_file:
                if 'viper-' in csv_file:
                    method_name = 'ViPER'
                    color = '#14517c'
                elif '3layers' in csv_file:
                    method_name = 'ViPER w/ 3-layer encoder'
                    color = '#009E73'
                elif 'noencoder' in csv_file:
                    method_name = 'ViPER w/o encoder'
                    color = '#f05260'
                else:
                    raise ValueError('Unknown method')
                print(csv_file)
            ...
            csv_file = 'results/ablation-encoder/' + csv_file
            trajectory_history = pd.read_csv(csv_file)
            trajectory_history = trajectory_history.sort_values('Step')
            trajectory_mean = trajectory_history.rolling(200, on='Step', min_periods=20, center=True).mean()
            trajectory_std = trajectory_history.rolling(200, on='Step', min_periods=20, center=True).std()
